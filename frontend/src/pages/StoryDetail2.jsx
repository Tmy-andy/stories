      ...
      <div className="layout-content-container flex flex-col w-full max-w-4xl flex-1">
        ...
        <div className="flex flex-col sm:flex-row-reverse gap-4 p-4">
          <div className="flex w-full flex-col sm:flex-row gap-3 sm:w-auto">
            {chapters.length > 0 && <Link to={`/chapter/${chapters[0]._id}`} className="flex items-center justify-center h-11 px-5 bg-primary text-white text-sm font-bold hover:bg-primary/90 transition-colors rounded-lg">Đọc từ đầu</Link>}
            <button className="flex items-center justify-center h-11 px-5 bg-gray-200 dark:bg-white/10 text-gray-800 dark:text-white text-sm font-bold hover:bg-gray-300 dark:hover:bg-white/20 transition-colors rounded-lg">Thêm vào giá sách</button>
          </div>
        ...
        <div className="mt-8">
          ...
          <div className="flex flex-col">
            {currentChapters.length > 0 ? currentChapters.map((chapter) => (
              <Link key={chapter._id} to={`/chapter/${chapter._id}`} className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">
                <div className="flex flex-col"><p className="text-gray-800 dark:text-gray-200 text-base font-medium">Chương {chapter.chapterNumber}: {chapter.title}</p><p className="text-gray-500 dark:text-gray-400 text-sm">Ngày đăng: {new Date(chapter.createdAt).toLocaleDateString('vi-VN')}</p></div>
